...
public:
    ...
    CSolidCone(ptr_shader_t pShader, const Vec3f& origin = Vec3f::all(0), float radius = 1, float height = 1, size_t sides = 24)
    {
        ...
        for (size_t s = 0; s < sides; s++) {
            ...
            /*
                Since we use triangles on each side of the cone, we get only one
                normal on each side, which leads to better shading effect closer to
                centre of cone rather than to the top. This can be solved by using more
                triangles. The other way to solve it is to use quadrons and subdivide each
                side of the cone at different heights and get more than one normal per side.
            */
