	...
	CSolidCone(ptr_shader_t pShader, const Vec3f& origin = Vec3f::all(0), float radius = 1, float height = 1, size_t sides = 24) 
	{
		...
		for (size_t s = 0; s < sides; s++) {
			...
			// --- PUT YOUR CODE HERE ---
                ...
                const Vec3f top(0, height, 0);                // This is the top point
                Vec3f dir0(1, 0, 0);
                Vec3f p0 = origin + radius * dir0;

                const Vec3f slope(0, radius / height, 0);    // This is the Slope
                Vec3f n0 = normalize(dir0 + slope);
                Vec3f dir1, p1, n1;
                
                float t0 = 0;                                // This is the initial texture coordinate
            int height_segments = sides/2;
            
            for (size_t s = 0; s < sides; s++) {
                    float t1 = static_cast<float>(s + 1) / sides;
                    float alpha = -2 * Pif * t1;
                    dir1 = Vec3f(cosf(alpha), 0, sinf(alpha));
                    p1 = origin + radius * dir1;
